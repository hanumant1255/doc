...
exports.checkUser = (email, password) =>
    ...
    exports.registerUser = (name, email, password) => 
        ...
        new Promise((resolve,reject) => {
            ...
            .then(user => resolve({ status: 201, message:user }))
...
exports.saveUserState = (cardHolder, cardValue, userFound) => 
    ...
    new Promise((resolve, reject) => {
        ...
        if (user.staticcardHolder.indexOf(cardHolder) == -1) {
            ...
            reject({ status: 400 , message: 'Invalid card holder sent !' });

        }
        else if (cardHolder!==cardValue.split("_")[0]) {
            
            reject({ status: 400 , message: 'Invalid color card sent in  holder  !' });
        }
        
        else if (user.staticCards.indexOf(cardValue.split("_")[1]) == -1) {
           
            reject({ status: 400 , message: 'Invalid card sent !' });
        }

        else if(userFound['no_of_cards']==52){

        reject({ status: 405 , message: 'Game has been finished....refresh to start new !' });

       }
        else if (userFound["cards"][cardHolder][cardValue] == true) {

            reject({ status: 405 , message: 'Card already present !' });
        } 
        ...
        user.findOneAndUpdate({ email: userFound.email }, userFound, { new: true })    

            .then(updatedUser => {                  
                resolve({ status: 201, message: updatedUser});              
            })
            .catch(err => reject({ status: 500, message: 'Internal Server Error !' }));
        }
    }); 
